...
const ChatInterface = () => {
  ...
  return (
    <div className="flex h-screen bg-gradient-to-r from-[#040404] to-[#2d8cff]">
      {/* Sidebar */}
      <div className={`transition-all duration-300 bg-gray-800 p-2 pt-5 flex flex-col h-screen ${isSidebarExpanded ? 'w-1/4' : 'w-16'}`}>
        <button onClick={toggleSidebar} className="text-gray-300 focus:outline-none">
          <svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" viewBox="0 0 24 24" className='border-2 bg-gray-700 border-gray-600 rounded-full p-2'>
            <path fill="none" stroke="white" stroke-linecap="round" stroke-width="1.5" d="M20 7H4m16 5H4m16 5H4" />
          </svg>
        </button>
        <div className="">
          <svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" viewBox="0 0 24 24" className='mt-5 border-2 bg-gray-700 border-gray-600 rounded-full p-2'>
            <path fill="white" d="M11 13H5v-2h6V5h2v6h6v2h-6v6h-2z" />
          </svg>
          <svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" viewBox="0 0 24 24" className='mt-[450px]'>
            ...
            <div className='ml-3'>
              ...
              <div className='flex items-center gap-2 mt-1'>
                <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                  <path fill="#34A853" d="M12 22q-2.075 0-3.9-.788t-3.175-2.137T2.788 15.9T2 12t.788-3.9t2.137-3.175T8.1 2.788T12 2t3.9.788t3.175 2.137T21.213 8.1T22 12t-.788 3.9t-2.137 3.175t-3.175 2.138T12 22" />
                </svg>
          ...
          <div style={{ position: 'relative', display: 'inline-block' }}>
            <div onClick={toggleDropdown} style={{ cursor: 'pointer' }}>
              <svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" viewBox="0 0 24 24">
                <path fill="white" fillRule="evenodd" d="M12 4a8 8 0 0 0-6.96 11.947A4.99 4.99 0 0 1 9 14h6a4.99 4.99 0 0 1 3.96 1.947A8 8 0 0 0 12 4m7.943 14.076q.188-.245.36-.502A9.96 9.96 0 0 0 22 12c0-5.523-4.477-10-10-10S2 6.477 2 12a9.96 9.96 0 0 0 2.057 6.076l-.005.018l.355.413A9.98 9.98 0 0 0 12 22q.324 0 .644-.02a9.95 9.95 0 0 0 5.031-1.745a10 10 0 0 0 1.918-1.728l.355-.413zM12 6a3 3 0 1 0 0 6a3 3 0 0 0 0-6" clipRule="evenodd" />
              </svg>
            </div>
            {isOpen && (
              <div className="absolute top-14 right-0 bg-gray-800 shadow-lg p-2 px-3 rounded-md z-50" onClick={logOut}>
        ...
        {showPrompts && (
          <div>
            <div className="pl-10 pb-2 bg-gray-900 flex flex-col items-center">
              ...
              <p className="text-gray-400 mt-2 md:text-5xl text-2xl">How can I help you today?</p>
            </div>
            <div className="px-5 pt-5">
              <div className="grid md:grid-cols-4 grid-cols-2 gap-4">
                {examplePrompts.map((prompt, index) => (
                  ...
                  >
                    <p className="text-gray-300 md:text-base text-xs">{prompt}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div ref={chatBoxRef} className="flex-grow p-5 overflow-y-auto">
          {messages.map((message, index) => (
            <div key={index} className={`mb-4 ${message.sender === 'user' ? 'text-right' : 'text-left'}`}>
              <div className={`inline-block p-4 rounded-lg ${message.sender === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-200'}`}>
                <p>{message.text}</p>
                {message.sender === 'bot' && (
                  <>
                    <p className="mt-2 text-sm"><span className='font-bold'>Description:</span> {message.title}</p>
                    <p className="mt-2 text-sm"><span className='font-bold'>Section:</span> {message.section}</p>
                    <p className="text-sm"><span className='font-bold'>Punishment:</span> {message.punishment}</p>
                  </>
                )}
              </div>
            </div>
            ...
            >
              {loading ? (
                <svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24">
                  <path fill="#ffffff" d="M12 2A10 10 0 1 0 22 12A10 10 0 0 0 12 2Zm0 18a8 8 0 1 1 8-8A8 8 0 0 1 12 20Z" opacity="0.5"/>
                  <path fill="#ffffff" d="M20 12h2A10 10 0 0 0 12 2V4A8 8 0 0 1 20 12Z">
